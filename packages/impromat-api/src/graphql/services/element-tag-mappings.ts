 ...
 * @returns
 */
export function transformImprowikiDeTags(
  tagNames: string[],
  languageCode: string | undefined,
) {
  if (!languageCode || languageCode === 'de') {
    let newTags = tagNames.map((tag) =>
      tag
...
export function transformImprowikiDeTag(
  tagName: string,
  languageCode: string | undefined,
) {
  return transformImprowikiDeTags([tagName], languageCode)?.tags ?? [tagName];
}
