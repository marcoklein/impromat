...
export class ElementVariationsService {
  ...
  async detectVariations(
    ...
    const variations = await this.elementLLMService.run({
      ...
      // TODO could extract a "Main Beschreibung" und dann davon "kleiner" Varianten
      // Koennte zum Beispiel eine zweite Prompt sein: Beschreibe den Ablauf den der folgende Text beschreibt.
      systemPromptDe: `Deine Aufgabe ist es alle Varianten welche der folgende Text beschreibt auszulesen. Die Variante muss aktiv erwähnt sein. Nutze ausschließlich im Text beschriebene Informationen! Sammle zuerst alle Varianten Namen in einer kurzen Liste. Gib danach die Varianten als JSON zurück: \`[{ "Name": "Titel der Variante aus dem Text", "Beschreibung": "Beschreibung der Variante aus dem Text" }, { "Name": "Titel der Variante aus dem Text", "Beschreibung": "Beschreibung der Variante aus dem Text" }]\``,
